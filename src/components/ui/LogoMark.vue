...
// The full-colour clover, matching public/favicon.svg — godotengine.org puts its real logo in the
// navbar rather than a flat silhouette. The white outline keeps it legible over the dark hero and
// over light page chrome alike, so callers don't need to recolour it.
// Gradient ids must be unique per instance and stable across SSR/hydration — these pages prerender.
const id = useId();
</script>

<template>
	<!-- "Clover" by Lorc (game-icons.net), CC BY 3.0 -->
	<svg viewBox="0 0 512 512" aria-hidden="true" focusable="false" class="shrink-0">
		<radialGradient :id="id">
			<stop offset="0" stop-color="#4a90e2" />
			<stop offset="1" stop-color="#48baff" />
		</radialGradient>
		<path
			:fill="`url(#${id})`"
			stroke="#fff"
